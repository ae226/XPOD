...
function renderSuggestions(workout) {
  ...
  for (const name of activeExercises) {
    ...
    const insight = buildOverloadInsight(name, sessions, plannedReps.get(name));
    const li = document.createElement("li");
    li.className = insight.level;
    li.innerHTML = `<strong>${escapeHtml(name)}</strong><br>${escapeHtml(insight.headline)}<div class="meta">${escapeHtml(insight.meta)}</div>`;
    list.appendChild(li);
  }
}
...
/**
 * Produce session-aware overload guidance with an exact next-weight target.
 * Compares the current session's best working set against the prior SESSION
 * (not the prior set), and recommends a concrete load for next time.
 */
function buildOverloadInsight(name, sessions, plannedReps) {
  const curr = sessions.at(-1);
  const prev = sessions.length >= 2 ? sessions.at(-2) : null;
  const b = curr.best;
  const target = parseRepTarget(plannedReps) ?? b.reps; // no plan → hold/beat achieved reps
  const inc = exerciseIncrement(name);
  const rpe = b.rpe;

  // Trend vs previous session, for context.
  let trend = "";
  if (prev) {
    ...
    else trend = "Roughly matched last session.";
  }

  // Deload check: two consecutive declines across the last three sessions.
  if (sessions.length >= 3) {
    ...
    if (e1Down || volDown) {
      ...
      return {
        level: "warn",
        headline: `Possible fatigue — performance down two sessions running.`,
        meta: `${volDown && drop > 0 ? `Volume off ~${drop}%. ` : ""}Consider a deload: ${roundToPlate(b.weight * 0.9)} lb next session, then rebuild.`,
      };
    }
  }

  // RPE-gated recommendation.
  if (rpe != null && rpe >= 9.5) {
    return {
      level: "warn",
      headline: `Hold ${fmtNum(b.weight)} lb — last set hit RPE ${rpe}.`,
      meta: `Repeat the load and bank a cleaner session before adding weight. ${trend}`,
    };
  }
  if (b.reps >= target && (rpe == null || rpe <= 8)) {
    const next = roundToPlate(b.weight + inc);
    return {
      level: "good",
      headline: `Next time: ${name} ${fmtNum(next)} lb × ${target}.`,
      meta: `You hit ${fmtNum(b.weight)} × ${b.reps}${rpe != null ? ` @ RPE ${rpe}` : ""}. ${trend}`.trim(),
    };
  }
  if (b.reps < target) {
    return {
      level: "warn",
      headline: `Repeat ${fmtNum(b.weight)} lb and chase ${target} reps.`,
      meta: `Got ${b.reps}/${target} last set. Lock in the rep target before loading up. ${trend}`.trim(),
    };
  }
  // Reps met but RPE in the 8.5–9.5 grind zone: hold load, add a rep.
  return {
    level: "good",
    headline: `Hold ${fmtNum(b.weight)} lb, add one rep next time.`,
    meta: `${b.reps} reps at RPE ${rpe} — earn the rep before the plate. ${trend}`.trim(),
  };
}
